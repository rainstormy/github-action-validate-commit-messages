...
export function legacyV1AcknowledgedAuthorEmailAddresses({
	...
	function hasAcknowledgedAuthorEmailAddress(author: LegacyV1UserIdentity): boolean {
		return author.emailAddress !== null && acknowledgedEmailAddressRegex.test(author.emailAddress)
	}

	return {
		key: "acknowledged-author-email-addresses",
		getInvalidCommits: (refinedCommits: LegacyV1Commits): LegacyV1Commits =>
			refinedCommits.filter(({ author }) => !hasAcknowledgedAuthorEmailAddress(author)),
	}
}
