...
export function legacyV1AcknowledgedCommitterEmailAddresses({
	...
	function hasAcknowledgedCommitterEmailAddress(committer: LegacyV1UserIdentity): boolean {
		return (
			committer.emailAddress !== null && acknowledgedEmailAddressRegex.test(committer.emailAddress)
		)
	}
	...
	return {
		...
		getInvalidCommits: (refinedCommits: LegacyV1Commits): LegacyV1Commits =>
			refinedCommits.filter(({ committer }) => !hasAcknowledgedCommitterEmailAddress(committer)),
	}
}
