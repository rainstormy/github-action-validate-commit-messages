...
describe.each`
	...
	${"Teach the parser to carry tiny snippets"} | ${'The recipe starts here.\n```js\nconst snack = "waffle"\nconsole.log(snack)\n```\nAnd then the prose comes back.'}                                     | ${[[rawText("The recipe starts here.")], [fencedCodeBlock("```js")], [fencedCodeBlock('const snack = "waffle"')], [fencedCodeBlock("console.log(snack)")], [fencedCodeBlock("```")], [rawText("And then the prose comes back.")]]}
	${"let the empty block breathe"}             | ${"Before\n```\n```\nAfter"}                                                                                                                             | ${[[rawText("Before")], [fencedCodeBlock("```")], [fencedCodeBlock("```")], [rawText("After")]]}
	...
	${"Tokenise two separate code postcards"}    | ${"```ts\nconst first = true\n```\nMiddle words.\n```sh\necho second\n```"}                                                                              | ${[[fencedCodeBlock("```ts")], [fencedCodeBlock("const first = true")], [fencedCodeBlock("```")], [rawText("Middle words.")], [fencedCodeBlock("```sh")], [fencedCodeBlock("echo second")], [fencedCodeBlock("```")]]}
	${"unclosed note fenced"}                    | ${'Intro\n```json\n{ "verified": false }\nRefs: #9001'}                                                                                                  | ${[[rawText("Intro")], [fencedCodeBlock("```json")], [fencedCodeBlock('{ "verified": false }')], [fencedCodeBlock("Refs: #9001")]]}
...
describe.each`
	subjectLine                                     | body                                                                                                                                                                                                                                    | expectedBodyLines
	${"nested fences inside bigger fences"}         | ${"````markdown\n```ts\nconst nested = true\n```\n````\nBack to ordinary markdown."}                                                                                                                                                    | ${[[fencedCodeBlock("````markdown")], [fencedCodeBlock("```ts")], [fencedCodeBlock("const nested = true")], [fencedCodeBlock("```")], [fencedCodeBlock("````")], [rawText("Back to ordinary markdown.")]]}
	...
	${"Close a tiny fence with a bigger one"}       | ${'```\nconst cake = "tiny"\n````\nRegular: not a trailer because prose follows\nstill regular'}                                                                                                                                        | ${[[fencedCodeBlock("```")], [fencedCodeBlock('const cake = "tiny"')], [fencedCodeBlock("````")], [rawText("Regular: not a trailer because prose follows")], [rawText("still regular")]]}
