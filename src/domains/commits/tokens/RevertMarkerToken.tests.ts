...
describe.each`
	...
	${'Revert ""'}                                                                     | ${[revert("Revert"), space(6), punctuation('"', 7), punctuation('"', 8)]}
	${'Revert " "'}                                                                    | ${[revert("Revert"), space(6), punctuation('"', 7), space(8), punctuation('"', 9)]}
	${'Revert "Revert """'}                                                            | ${[revert("Revert"), space(6), punctuation('"', 7), revert("Revert", 8), space(14), punctuation('"', 15), punctuation('"', 16), punctuation('"', 17)]}
	...
	${'Revert "Revert "Repair the soft ice machine""'}                                 | ${[revert("Revert"), space(6), punctuation('"', 7), revert("Revert", 8), space(14), punctuation('"', 15), word("Repair", 16), space(22), word("the", 23), space(26), word("soft", 27), space(31), word("ice", 32), space(35), word("machine", 36), punctuation('"', 43), punctuation('"', 44)]}
	${'Revert "Revert "Revert "Repair the soft ice machine"""'}                        | ${[revert("Revert"), space(6), punctuation('"', 7), revert("Revert", 8), space(14), punctuation('"', 15), revert("Revert", 16), space(22), punctuation('"', 23), word("Repair", 24), space(30), word("the", 31), space(34), word("soft", 35), space(39), word("ice", 40), space(43), word("machine", 44), punctuation('"', 51), punctuation('"', 52), punctuation('"', 53)]}
...
describe.each`
	...
	${'Revert ""weirdly quoted message'}                          | ${[revert("Revert"), space(6), punctuation('"', 7), punctuation('"', 8), word("weirdly", 9), space(16), word("quoted", 17), space(23), word("message", 24)]}
	...
	${'revert "a mere bugfix attempt"""'}                         | ${[revert("revert"), space(6), punctuation('"', 7), word("a", 8), space(9), word("mere", 10), space(14), word("bugfix", 15), space(21), word("attempt", 22), punctuation('"', 29), punctuation('"', 30), punctuation('"', 31)]}
`(
...
`(
	...
	(props: { subjectLine: string }) => {
		...
		it("does not extract any revert tokens", () => {
			const commit = mapCrudeCommitToCommit(crudeCommit, configuration)
			expect(commit.subjectLine.some(isToken("revert"))).toBe(false)
		})
	},
)
