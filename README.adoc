...
. Write unit tests for the rule logic in `src/rules/Rule.tests.ts`.
...
Implement the rule logic to make the unit tests pass.
...
. Add a detection message and a hint to the reporter in `src/validator/Reporter.ts` to make the unit tests pass.
